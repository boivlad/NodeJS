...
const signIn = (req, res) => {
	...
	User.findOne({
		...
		.then((user) => {
			...
			if (user.status != "1") {
				res.status(401).json({
					message: 'Ваш аккаунт не активирован, проверьте почту'
				});
			} else {
		...
		.then((user) => {
			if (!user) {
				if (login.match(reg)[0] !== login)
					res.status(401).json({
						message: 'Вы ввели логин неверного формата'
					});
				else {
					if (password.match(reg)[0] !== password)
						res.status(401).json({
							message: 'Вы ввели пароль неверного формата'
						});
					else {
						User.create({
							login: login,
				...
				res.status(401).json({
					message: 'Пользователь под данной почтой или именем пользователя уже зарегистрирован!'
				});
			}
		})
